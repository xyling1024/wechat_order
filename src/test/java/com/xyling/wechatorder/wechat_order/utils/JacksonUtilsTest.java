...
public class JacksonUtilsTest {
    ...
    public void json2Object() throws Exception {
        ...
        List list = JacksonUtil.json2Object(jsonListStr, List.class);
        log.info("list = {}", list);
        log.info("list.get(0) instanceof Map? {}", list.get(0) instanceof Map);    // 默认转化出来为List<Map>类型

        // 复杂类型的json转换
        List orderDetailList = JacksonUtil.json2Object(jsonListStr, List.class, OrderDetail.class);
       ...
       /* String listStr = JacksonUtil.object2Json(list);
        log.info("listStr = {}", listStr);

        String jsonMapStr = "{productId:222222, productQuantity:1}";
        Map map = JacksonUtil.json2Object(jsonMapStr, Map.class);
        log.info("map = {}", map);

        String mapStr = JacksonUtil.object2Json(map);
